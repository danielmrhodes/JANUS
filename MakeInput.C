void MakeInput() {
  ...
  const double eMax = 322.0;
  const double eMin = 265.0;
  const int nBinsE = 10; //Nmber of energy meshpoints

  const int nBinsT = 30; //Number of scattering angle meshpoints

  const std::string input = "cd106.txt"; //Cygnus nucleus file, must already exist
